...
// Hot Pepper API 요청 처리 / Hot Pepper APIのリクエスト処理
app.get('/api/restaurants', async (req, res) => {
    try {
        ...
        console.error('API 요청 중 오류 발생:', error); // APIリクエスト中にエラーが発生しました: 
        res.status(500).json({ message: '서버 에러 발생' }); // サーバーエラーが発生しました
    }
});

// Google Maps API 키를 클라이언트에 전달하는 엔드포인트 / Google Maps APIキーをクライアントに渡すエンドポイント
...
app.listen(PORT, () => {
    console.log(`Server is running on port ${PORT}`); // サーバーはポート${PORT}で実行中です
});
